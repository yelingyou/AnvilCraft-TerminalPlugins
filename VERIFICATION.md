...
`TerminalPluginPanel#dispatch` 里，只有 `SET_FILTER_SLOT` / `SET_ALCHEMY_FILTER` 会把**光标上的物品**作为 payload 发出去，
`SET_SAMPLE_SLOT` 不在这个判断里 —— 于是发到服务端的 payload 永远是空栈，服务端每次都把槽清空，表现就是「插件无法标记物品」。
现在三个 sample 动作都走同一条 payload 分支。

### 2. 锻造插件改成三个槽（对齐皇家锻造台）

`PluginSample` 从「主槽 + 输出」扩成「**工具(基底) / 金属(附加物) / 模板** + 输出」四个字段：

| 槽 | 作用 | 是否消耗 |
|---|---|---|
| 工具 / 基底 | 指定要锻造哪件工具，留空则按过滤表自己挑 | 消耗 |
| 金属 / 附加物 | 指定附加物（下界合金锭等） | 消耗 |
| 模板 | 指定锻造模板 | **不消耗**（皇家锻造台语义） |
| 输出 | 只读，显示上次锻造产物 | —— |

槽位内容没填时会自动从终端存储里找满足配方的物品；填了之后必须**既满足配方判定、又确实在存储里**才会用。
执行页的锻造视图按「工具 / 金属 / 模板 / 输出」四行排布，每行右侧一个 16×16 槽。
