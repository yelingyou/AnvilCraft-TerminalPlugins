...
| 铁砧修复 | 每材料修复点数 0(1/4 耐久) → 25 → 50 → 100 → 200 → 500 | —— |
...
| **铁砧修复插件** | 用存储里匹配过滤的材料修复存储中受损的装备 | 每次消耗 1 个材料，修复 25/50/100/200/500 点耐久（默认按最大耐久的 1/4） |

> **过滤表语义**：*消耗 / 破坏型*插件（销毁、压缩）留空时**什么都不做**（必须显式列出允许处理的物品，避免误删误压）；
...
- [x] 销毁(Void)、压缩、铁砧修复、流体接口、工具切换、生物捕捉、经验泵
- [ ] 合成扩展：本体「合成窗口」只有工作台 + 切石机，**锻造台**还是空缺，计划做成锻造插件
